...
# -*- coding: utf-8 -*-
"""
qc_thresholds_autoscan_unified_pro_ml_rev8_0_shotaware.py
---------------------------------------------------------
STRICT superset of rev4.0 (원코드 삭제/수정 없음, 기능만 추가).

신규 (rev8.0 - ShotAware + ML Cross-Check 강화):
9) ML 학습 시 K-fold 교차검증/샷 그룹 분리 통계를 출력하고 모델 JSON에 학습 요약을 저장.
10) CLI 분류 결과에 STRICT/FLEX/SHOT 임계 기준과 가드 설정을 요약해 “양품인데 왜 FAIL인지”를 설명.
...
README_TEXT = r"""
[rev8.0 ShotAware README]
의존성: python3, numpy, tkinter, matplotlib (GUI용). CLI에서는 --no_gui와 함께 사용 가능.

실행 흐름:
1) GUI 실행: python qc_thresholds_autoscan_unified_pro_ml_rev8_0_shotaware.py
...
3) 모델 학습: python ... fit --good_root GOOD --bad_root BAD --out model.json [--cv_folds 5 --cv_by_shot]
   - JSON에는 Y11/S11 골든과 임계치, 피처 파라미터가 저장되며 classify에서 --ml_model_path로 사용.
   - --cv_folds를 2 이상으로 주면 K-fold 교차검증을 수행하고 fold별/전체 혼동행렬, FP/FN, 정확도를 출력.
   - --cv_by_shot를 함께 지정하면 동일 shot_id는 한 fold에만 배치되어 데이터 누수를 줄입니다.
- rev8 ML은 r/rmse + 노이즈/피크 특징에 notch/선형성/좌우 비대칭/피크 이동량까지 포함하여 왜곡 샘플을 정밀 분리.
- ML 데이터셋 가이드: GOOD/BAD 각각 최소 **30개 이상**(권장 50~80개)을 준비하고, 가능한 한 서로 다른 샷에서
  ...
  교차검증 리포트에서 fold 간 편차가 크다면 샷 균형을 더 맞추거나 특징치를 추가 확보해야 합니다.
...
- rev8 CLI는 STRICT/FLEX/SHOT 임계값, 가드 토글, relax 배수, ML/S11 모드 등을 한 줄로 요약해 기준을 명확히 안내합니다.
"""
...
def _build_cv_folds(num_items: int, k: int, seed: int) -> List[np.ndarray]:
    rng = np.random.default_rng(seed)
    idx = np.arange(num_items)
    if k <= 1 or num_items < k:
        return [idx]
    rng.shuffle(idx)
    return [idx[i::k] for i in range(k)]


def _build_grouped_cv_folds(groups: List[str], k: int, seed: int) -> List[np.ndarray]:
    rng = np.random.default_rng(seed)
    group_to_indices: Dict[str, List[int]] = {}
    for i, g in enumerate(groups):
        group_to_indices.setdefault(g, []).append(i)
    keys = list(group_to_indices.keys())
    if not keys:
        return _build_cv_folds(len(groups), k, seed)
    rng.shuffle(keys)
    folds: List[List[int]] = [[] for _ in range(max(1, min(k, len(keys))))]
    for idx, key in enumerate(keys):
        folds[idx % len(folds)].extend(group_to_indices[key])
    return [np.array(sorted(f), dtype=int) for f in folds if f]


def _summarize_confusion(tp: int, fp: int, fn: int, tn: int) -> Dict[str, float]:
    total = tp + fp + fn + tn
    acc = (tp + tn) / total if total else 0.0
    precision = tp / (tp + fp) if (tp + fp) else 0.0
    recall = tp / (tp + fn) if (tp + fn) else 0.0
    return dict(tp=tp, fp=fp, fn=fn, tn=tn, accuracy=acc, precision=precision, recall=recall)
...
def fit_ml_model(good_root: str, bad_root: str, domain: str, compare_mode: str,
                 ...
                 smooth_win:int=31, cv_folds: int=0, cv_seed: int=13,
                 cv_by_shot: bool=False, shot_regex: Optional[str]=None) -> Dict:
    ...
    shot_pat = shot_regex if (shot_regex and shot_regex.strip()) else regex
    for p in good_files:
        sp = parse_s1p(p)
        feats = _compute_basic_features(sp, g_freq_g, g_curve_g, g_freq_b, g_curve_b,
                                        fmin, fmax, domain, compare_mode, smooth_win)
        vec = np.array([feats.get(name, 0.0) for name in feature_order], float)
        Xg.append(vec)
        sid = extract_shot_id(p, shot_pat) if cv_by_shot else "GOOD"
        dataset.append(dict(path=p, label=1, features=vec, shot=sid))
        shot_groups.append(sid)
    for p in bad_files:
        sp = parse_s1p(p)
        feats = _compute_basic_features(sp, g_freq_g, g_curve_g, g_freq_b, g_curve_b,
                                        fmin, fmax, domain, compare_mode, smooth_win)
        vec = np.array([feats.get(name, 0.0) for name in feature_order], float)
        Xb.append(vec)
        sid = extract_shot_id(p, shot_pat) if cv_by_shot else "BAD"
        dataset.append(dict(path=p, label=0, features=vec, shot=sid))
        shot_groups.append(sid)
    ...
    training_report: Dict[str, object] = dict(
        total_good=len(good_files), total_bad=len(bad_files), feature_names=feature_order
    )
    if cv_folds and cv_folds >= 2:
        if cv_by_shot:
            folds = _build_grouped_cv_folds(shot_groups, cv_folds, cv_seed)
        else:
            folds = _build_cv_folds(len(dataset), cv_folds, cv_seed)
        cv_results = []
        print(f"[교차검증] folds={len(folds)}, by_shot={cv_by_shot}, seed={cv_seed}")
        for i, fold_idx in enumerate(folds, 1):
            test_mask = np.zeros(len(dataset), dtype=bool)
            test_mask[fold_idx] = True
            train_idx = np.where(~test_mask)[0]
            if not len(train_idx):
                continue
            Xg_train = [dataset[j]['features'] for j in train_idx if dataset[j]['label']==1]
            Xb_train = [dataset[j]['features'] for j in train_idx if dataset[j]['label']==0]
            if not Xg_train or not Xb_train:
                continue
            lda_fold = _fit_lda(np.vstack(Xg_train), np.vstack(Xb_train))
            w = np.asarray(lda_fold["w"], float); c = float(lda_fold["c"])
            tp=fp=fn=tn=0
            for j in fold_idx:
                row = dataset[j]
                z = _lda_score(row['features'], w, c)
                pred_good = z >= 0
                if row['label'] == 1:
                    if pred_good: tp += 1
                    else: fn += 1
            ...
            stats = _summarize_confusion(tp, fp, fn, tn)
            stats.update(dict(fold=i, test_size=int(len(fold_idx))))
            cv_results.append(stats)
            print(f"  fold{i}: test={len(fold_idx)} TP={tp} FP={fp} FN={fn} TN={tn} | acc={stats['accuracy']:.3f} precision={stats['precision']:.3f} recall={stats['recall']:.3f}")
        if cv_results:
            avg = dict(tp=sum(r['tp'] for r in cv_results),
                       fp=sum(r['fp'] for r in cv_results),
                       fn=sum(r['fn'] for r in cv_results),
                       tn=sum(r['tn'] for r in cv_results))
            avg.update(_summarize_confusion(avg['tp'], avg['fp'], avg['fn'], avg['tn']))
            avg['folds'] = len(cv_results)
            training_report['cv'] = dict(folds=cv_results, aggregate=avg)
            print(f"[교차검증 요약] TP={avg['tp']} FP={avg['fp']} FN={avg['fn']} TN={avg['tn']} | acc={avg['accuracy']:.3f} precision={avg['precision']:.3f} recall={avg['recall']:.3f}")
        else:
            training_report['cv'] = dict(message='유효한 교차검증 fold를 구성하지 못했습니다.')
            print("[교차검증] 유효한 fold 구성이 불가능하여 보고서를 생략합니다.")
...
def _summarize_stage_counts(results: List[QCResult]) -> str:
    if not results:
        return ""
    from collections import Counter
    cnt = Counter(r.stage for r in results)
    return ", ".join(f"{stage}:{cnt[stage]}" for stage in sorted(cnt))


def print_cli_threshold_summary(thresholds: Tuple[float, float], args, results: List[QCResult]):
    r_min, rmse_max = thresholds
    strict_part = f"STRICT r≥{r_min:.4f} rmse≤{rmse_max:.3f}dB" if (r_min is not None and rmse_max is not None) else "STRICT auto"
    flex_part = f"FLEX x{args.relax_factor:.2f}" if args.use_two_stage else "FLEX off"
    shot_part = f"SHOT relax x{args.shot_relax:.2f}" if args.shot_aware else "SHOT off"
    ml_part = f"ML {args.ml_mode} ({'ON' if args.ml_model_path else 'OFF'})"
    s11_part = f"S11 {args.s11_cc_mode if args.s11_crosscheck else 'OFF'}"
    guard_bits = []
    if args.use_noise_gate:
        guard_bits.append(f"Noise≤{args.roughness_max_db:.3f}dB")
    if args.use_shape_gate:
        guard_bits.append(f"Shape prom≥{args.min_prom_db:.2f}dB")
    guard_str = ", ".join(guard_bits) if guard_bits else "가드 OFF"
    stage_str = _summarize_stage_counts(results)
    print(f"[기준 요약] {strict_part} | {flex_part} | {shot_part} | {ml_part} | {s11_part}")
    print(f"            Guards: {guard_str}")
    if stage_str:
        print(f"            Stage counts: {stage_str}")
...
def launch_gui():
    ...
    win = tk.Tk()
    win.title("QC Thresholds — Pro+++++ ML rev8.0 (ML + 2-Stage + S11-CC)")
    ...
    shotfrm = ttk.LabelFrame(left, text="Shot-Aware / Shape Guards (rev8.0)")
    ...
    mlfrm = ttk.LabelFrame(left, text="ML / Cross-check (rev8.0)")
...
def main():
    p = argparse.ArgumentParser(description="QC Classifier Pro+++++ ML rev8.0 (ML + 2-Stage + S11-CC)")
    ...
    pf.add_argument('--shot_regex')
    pf.add_argument('--cv_folds', type=int, default=0)
    pf.add_argument('--cv_seed', type=int, default=13)
    pf.add_argument('--cv_by_shot', action='store_true')
    ...
    if args.cmd=='fit':
        model = fit_ml_model(args.good_root, args.bad_root, args.domain, args.compare_mode,
                             ...
                             args.cv_folds, args.cv_seed, args.cv_by_shot, args.shot_regex)
    ...
    if args.cmd=='classify':
        ...
        print_cli_threshold_summary((rmin, ermse), args, results)
