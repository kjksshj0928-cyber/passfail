...
# -*- coding: utf-8 -*-
"""
qc_thresholds_autoscan_unified_pro_ml_rev6_0_shotaware.py
---------------------------------------------------------
STRICT superset of rev4.0 (원코드 삭제/수정 없음, 기능만 추가).

신규 (rev6.0 - ShotAware + Noise Hardening):
...
3) Noise Guard 개선
   ...
   - rev6.0: Strict Noise Guard (Y11 곡선에 미세 노이즈도 허용하지 않을 때 사용) 추가.
4) ML & FLEX 연동 강화
...
ML 학습 절차 예시:
1. GOOD/BAD 각각 대표 샷이 충분한 폴더를 준비 (동일 shot이 섞이지 않도록 주의).
2. python qc_thresholds_autoscan_unified_pro_ml_rev6_0_shotaware.py fit \
      --good_root GOOD_SAMPLE_DIR --bad_root BAD_SAMPLE_DIR \
      --out qc_ml_model_rev6.json --freq_min 5.5e9 --freq_max 7.5e9
   - 필요한 경우 --regex 로 샷 패턴을 제한하고 --smooth_win 으로 노이즈 기준을 맞춘다.
...
- Shape/Linearity/Notch 강도는 rev6.0 강화 감시(이빨 빠짐, 노치, 평탄 착시)를 제어. 1.0이 기본, 더 크면 엄격.
- Strict Noise Guard 는 Y11 노이즈 허용 한계를 dB 단위로 지정하며, 초과 시 강제 FAIL (0 입력 시 비활성).
...
def noise_envelope_db(y_db: np.ndarray, smooth_win: int=31) -> float:
    base = _movavg(y_db, smooth_win)
    resid = y_db - base
    if resid.size == 0:
        return 0.0
    env = float(np.max(resid) - np.min(resid))
    return abs(env)
...
# Backward alias (rev5 model JSON을 불러올 때도 동일 배열 사용)
ML_FEATURE_NAMES_REV5 = ML_FEATURE_NAMES_REV6
...
def fit_ml_model(good_root: str, bad_root: str, domain: str, compare_mode: str,
    ...
    # features
    feature_order = list(ML_FEATURE_NAMES_REV6)
    Xg = []; Xb = []
    for p in good_files:
        sp = parse_s1p(p)
...
def predict_with_model(sp: S1P, model: Dict) -> Tuple[str, float, np.ndarray]:
    ...
    names = model.get("feature_names") or list(ML_FEATURE_NAMES_REV6)
...
class QCResult:
    ...
    noise_envelope_db: Optional[float] = None
...
def classify_folder(root: str, golden_mode: str, golden_path: Optional[str], fmin: Optional[float], fmax: Optional[float],
                    ...
                    strict_noise_db: float=0.12, strict_noise_mode: str='FAIL',
    ...
    strict_mode = (strict_noise_mode or 'FAIL').upper()
    ...
    for p, m in zip(s1p_files, mets):
        ...
        noise_env = None
        if y_sel.size >= 5:
            smooth_for_noise = int(max(5, smoothing_window_pts))
            noise_env = noise_envelope_db(y_sel, smooth_win=smooth_for_noise)
            if use_noise_gate:
                nr = noise_rms_db(y_sel, smooth_win=smooth_for_noise)
                if nr > roughness_max_db:
                    ...
                    if gate_mode.upper()=='FAIL':
                        base_decision = 'FAIL'
            if strict_noise_db is not None and strict_noise_db > 0 and noise_env is not None:
                if noise_env > strict_noise_db:
                    flag = f"STRICT_NOISE env={noise_env:.3f}dB>{strict_noise_db:.3f}dB"
                    notes.append(flag)
                    guard_flags.append(flag)
                    if strict_mode=='FAIL':
                        base_decision = 'FAIL'
        ...
        results.append(QCResult(
            ...
            noise_rms_db=nr, noise_envelope_db=noise_env, peak_prom_db=pr, fwhm_mhz=fw,
            gate_notes=gate_str,
...
def save_results_csv(root: str, results: List[QCResult], r_min: float, rmse_max: float, out_name="qc_results.csv") -> str:
    ...
    with open(out_path, 'w', newline='', encoding='utf-8') as f:
        ...
        w.writerow(["path","r","rmse_db","decision","r_min","rmse_max",
                    "noise_rms_db","noise_envelope_db","peak_prom_db","fwhm_mhz","gate_notes",
        ...
        for row in results:
            w.writerow([row.path, f"{row.r:.6f}", f"{row.rmse_db:.6f}", row.decision,
                        ...
                        ("" if row.noise_envelope_db is None else f"{row.noise_envelope_db:.6f}"),
...
def launch_gui():
    ...
    win = tk.Tk()
    win.title("QC Thresholds — Pro+++++ ML rev6.0 (ShotAware + Strict Noise)")
    ...
    def _run_classify(auto: bool):
        try:
            ...
            try: strict_noise_val = float(var_strict_noise.get() or "0.0")
            except Exception: strict_noise_val = 0.12
            strict_mode = strict_noise_mode_var.get()
            ...
            results, (used_r, used_e), (g_freq, g_curve), shot_map = classify_folder(
                ...
                smoothing_window_pts=g_smooth, roughness_max_db=g_rough,
                strict_noise_db=strict_noise_val, strict_noise_mode=strict_mode,
                min_prom_db=g_minprom, max_fwhm_mhz=g_maxfwhm,
            ...
            for row in results[:5]:
                noise_bits = []
                if row.noise_rms_db is not None:
                    noise_bits.append(f"noise={row.noise_rms_db:.3f}dB")
                if row.noise_envelope_db is not None:
                    noise_bits.append(f"env={row.noise_envelope_db:.3f}dB")
                noise_str = " ".join(noise_bits)
                _log(f"{row.decision}\t{row.stage}\tml={row.ml_label}:{row.ml_score:.2f}\ts11cc={row.s11_cc}\t"
                     f"r={row.r:.4f}\trmse={row.rmse_db:.3f}\t{noise_str}\tshot={row.shot_id}/{row.used_golden}\tguards={row.guards}\t{row.path}")
    ...
    ttk.Button(ctl, text="HELP", command=lambda: messagebox.showinfo("HELP",
        ...
        "S11 Cross-check는 Y11 도메인에서 S11도 정상인지 검증합니다. CLI README: python qc_thresholds_autoscan_unified_pro_ml_rev6_0_shotaware.py readme") ).grid(row=0, column=3, padx=(2,0))
    ...
    ttk.Combobox(gatefrm, textvariable=strict_noise_mode_var, values=("WARN","FAIL"), width=6, state="readonly").grid(row=5, column=3, sticky='w', padx=4)
...
def main():
    p = argparse.ArgumentParser(description="QC Classifier Pro+++++ ML rev6.0 (ShotAware + Strict Noise + ML)")
    ...
    if args.cmd=='classify':
        results, (rmin, ermse), _, _ = classify_folder(
            ...
            strict_noise_db=args.strict_noise_db, strict_noise_mode=args.strict_noise_mode,
        ...
        for row in results[:5]:
            ...
            if row.noise_envelope_db is not None: parts.append(f"noise_env={row.noise_envelope_db:.3f}dB")
